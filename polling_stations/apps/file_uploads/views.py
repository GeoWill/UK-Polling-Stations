        ...
        context["STATIONS"] = [
            {
                ...
                "example_uprn": UprnToCouncil.objects.filter(
                    ...
                    lad=council_from_logger_db.geography.gss,
                )
                .first()
                .uprn.uprn,
            }
            for station in Council.objects.using(DEFAULT_DB_ALIAS)
            .get(council_id=council_from_logger_db.council_id)
            .pollingstation_set.all()
        ]
        return context
